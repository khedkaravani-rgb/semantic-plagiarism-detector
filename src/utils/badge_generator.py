...
def generate_badge_png(
    ...
    # Inner border
    draw.rectangle(
        [border_width + 4, border_width + 4, width - border_width - 4, height - border_width - 4],
        outline='#ffffff',
        width=2
    )
    ...
    # Draw circle
    draw.ellipse(
        [check_x - check_size, check_y - check_size, check_x + check_size, check_y + check_size],
        fill='#22c55e',
        outline='#ffffff',
        width=4
    )
    ...
    if text_preview:
        preview_text = f"Verified: {text_preview[:80]}..." if len(text_preview) > 80 else f"Verified: {text_preview}"
